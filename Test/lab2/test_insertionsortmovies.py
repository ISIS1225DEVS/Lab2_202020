...
moviesDetailsFile = cf.data_dir + 'SmallMoviesDetailsCleaned.csv'


def setUp():
    print('Loading movies details')
...
def test_loading_CSV_y_ordenamiento():
    ...
    while not (lt.isEmpty(lst_moviesDetails)):
        x = int(lt.removeLast(lst_moviesDetails)['vote_average'])
        if not (lt.isEmpty(lst_moviesDetails)):
            y = int(lt.lastElement(lst_moviesDetails)['vote_average'])
        else:
            break
        assert x > y
